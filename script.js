...
// UPDATED: Using local audio files instead of a CDN
const AUDIO_FILES = {
    'A': 'audio/A4.mp3',
    'A#': 'audio/Bb4.mp3',
    'B': 'audio/B4.mp3',
...
const SCALE_DESCRIPTIONS = {
    major: { description: "The official scale of campfire songs and dad rock anthems.", artists: ["David Gilmour", "Eric Clapton", "Brian May"] },
    minor: { description: "For writing songs about your ex, even if you're happily married.", artists: ["Randy Rhoads", "Yngwie Malmsteen", "Michael Schenker"] },
    minorPentatonic: { description: "Congratulations, you now know 90% of all classic rock solos.", artists: ["Jimi Hendrix", "Jimmy Page", "B.B. King", "SRV"] },
    majorPentatonic: { description: "Sounds happy in any context. Use with caution at funerals.", artists: ["Dickey Betts", "Duane Allman", "Chuck Berry"] },
    blues: { description: "The minor pentatonic scale, but with a spicy little ✨mistake✨.", artists: ["Eric Clapton", "John Mayer", "B.B. King"] },
    ionian: { description: "Just a fancy way of saying 'Major Scale'. Sounds smarter though.", artists: ["Frank Zappa", "Joe Satriani", "Jeff Beck"] },
    dorian: { description: "The minor scale's cooler, more sophisticated older sibling.", artists: ["Carlos Santana", "Robby Krieger", "David Gilmour"] },
    phrygian: { description: "Play this to sound like a flamenco master in a metal band.", artists: ["Kirk Hammett", "Joe Satriani", "Yngwie Malmsteen"] },
    lydian: { description: "The major scale, but it went to space for a semester.", artists: ["Steve Vai", "Joe Satriani", "Tom Morello"] },
    mixolydian: { description: "The official scale of rock songs that are kinda bluesy but also happy.", artists: ["Jimi Hendrix", "Jerry Garcia", "Angus Young"] },
    aeolian: { description: "Just a fancy way of saying 'Natural Minor'. Go tell your friends.", artists: ["Michael Schenker", "Iron Maiden", "Judas Priest"] },
    locrian: { description: "Use this to scare your pets and confuse your audience.", artists: ["John Scofield", "Robert Fripp", "Meshuggah"] }
};
...
function displayChords(chords) {
    ...
    if (chords.length === 0) {
        chordsDisplay.innerHTML = '<p style="color: var(--text-dark);">No chords defined for this scale type.</p>';
        return;
    }
    chords.forEach(chord => {
        const chordChip = document.createElement('div');
        chordChip.className = 'chord-chip';
        chordChip.textContent = chord;
        chordsDisplay.appendChild(chordChip);
    });
}
...
function drawFretboardAndNotes() {
    ...
    if (info) {
        scaleDescription.innerHTML = `<p class="one-liner">${info.description}</p><p class="artist-list"><b>Used By:</b> ${info.artists.join(', ')}</p>`;
    } else {
        scaleDescription.innerHTML = "";
    }
    const scaleNotes = calculateScaleNotes(selectedKey, SCALE_FORMULAS[selectedScale]);
    const diatonicChords = calculateDiatonicChords(scaleNotes, selectedScale);
    displayChords(diatonicChords);
        ...
        for (let fret = 0; fret <= FRET_COUNT; fret++) {
            ...
            if (scaleNotes.includes(noteOnFret)) {
                ...
                if (fretElement) {
                    ...
                    noteDiv.textContent = noteOnFret;
                    if (noteOnFret === selectedKey) {
                        noteDiv.classList.add('root-note');
                    }
...
function generateFretboard() {
    ...
    GUITAR_TUNING.slice().reverse().forEach((stringNote) => {
        ...
        for (let fret = 0; fret <= FRET_COUNT; fret++) {
            ...
            fretDiv.dataset.stringNote = stringNote;
...
function getChordNotes(chordName) {
    const [rootNote, quality] = chordName.split(' ');
    const formula = CHORD_FORMULAS[quality];
    if (!formula) return [];
    const rootIndex = NOTES.indexOf(rootNote);
    return formula.map(step => NOTES[(rootIndex + step) % NOTES.length]);
}
            ...
            if (chordNotes.includes(noteOnFret)) {
                ...
                if (fretElement) {
                    ...
                    if (noteOnFret === root) {
                        ...
                        noteDiv.classList.add('root-note');
                    } else if (noteOnFret === third) {
                        noteDiv.textContent = '3';
                    } else if (noteOnFret === fifth) {
                        noteDiv.textContent = '5';
                    }
                    fretElement.appendChild(noteDiv);
                }
            }
